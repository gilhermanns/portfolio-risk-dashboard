...
A research dashboard for **quantifying and visualizing multi-asset portfolio risk** in **Wealth Management, Private Banking and Asset Management** contexts. It compares risk measures and historical stress behaviour so that portfolio vulnerabilities can be discussed with their assumptions visible.

## Business Relevance

For a portfolio-risk review, the dashboard supports the following questions:
...
The project provides a reproducible risk-review workflow rather than a live advisory or regulatory reporting system.
...
The resulting reports are intended to make model differences, concentration and historical stress behaviour easier to review with advisors and clients.
...
The following figures were generated by the packaged report pipeline from the repository's bundled market-data snapshot. The primary 12-asset analysis window is **1989-12-29 to 2018-04-11** with **7,378 trading days**; an extended factor-only window runs to 2026-06-29.

### Portfolio concentration and tail risk

| Metric | Result |
|---|---:|
| Portfolio Herfindahl-Hirschman Index | 0.1218 |
| Equal-weight 12-asset benchmark HHI | 0.0833 |
| Historical VaR, 95% confidence | 1.43% |
| Historical Expected Shortfall, 95% confidence | 2.16% |
| Historical VaR, 99% confidence | 2.49% |
| Historical Expected Shortfall, 99% confidence | 3.47% |

### Model comparison

| Confidence | Historical VaR | Normal VaR | Student-t VaR | Monte-Carlo VaR | Historical ES |
|---|---:|---:|---:|---:|---:|
| 95% | 1.43% | 1.49% | 1.41% | 1.50% | 2.16% |
| 99% | 2.49% | 2.13% | 2.39% | 2.14% | 3.47% |

This comparison is useful in a private-banking risk conversation because it makes model choice visible: at the 99% level, the historical tail estimate is higher than the Normal and Monte-Carlo estimates in this run, while Expected Shortfall quantifies the average loss beyond the VaR threshold.

### Full report

The complete generated HTML report, including portfolio composition, factor attribution, rolling risk metrics, correlation regimes, and stress-test sections, is available at [`reports/risk_report.html`](reports/risk_report.html). It is a reproducible analytical output rather than a live advisory dashboard.
...
This project is licensed under the MIT License. It is intended for educational and research purposes in quantitative finance. The linked figures were generated from the repository's bundled data snapshot and configuration; they are historical, configuration-specific and not a forecast. The project does not constitute financial advice.
